...
abstract contract STBTBase is IERC20, IERC20Metadata, IERC1644, Ownable {
    ...
    function getSharesByAmountRoundUp(uint256 _amount) public view returns (uint256 result) {
        ...
        return _totalSupply == 0 ? 0 : (_amount * totalShares_ + _totalSupply - 1) / _totalSupply;
    }

    function getSharesByAmount(uint256 _amount) public view returns (uint256 result) {
        return totalSupply_ == 0 ? 0 : _amount * totalShares_ / totalSupply_;
    }

    function getAmountByShares(uint256 _shares) public view returns (uint256 result) {
        return totalShares_ == 0 ? 0 : _shares * totalSupply_ / totalShares_;
    }
    ...
    function _checkSendPermission(address _sender) internal view {
    ...
    function _checkReceivePermission(address _recipient) internal view {
    ...
    function _burnSharesWithCheck(address _account, uint256 _shares) internal virtual returns (uint256 newTotalShares) {
        _checkSendPermission(_account);
        return _burnShares(_account, _shares);
    }

    function _burnShares(address _account, uint256 _shares) internal virtual returns (uint256 newTotalShares) {
        ...
        newTotalShares = totalShares_ - _shares;
